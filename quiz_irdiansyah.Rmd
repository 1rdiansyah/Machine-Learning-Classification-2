...
3. Pada model decision tree, tujuan dari mengatur `mincriterion = 0.90` adalah ...
  ...
  - [x] Untuk pemangkasan (*pruning*) pohon, hanya node dengan p-value <= 0.10 yang dapat melakukan *splitting node*
...
5. Berdasarkan ringkasan `model_rf$finalModel` di atas, bagaimana cara menginterpretasikan out-of-bag error dari model?
  ...
  - [x] Kita memiliki error 33.61% pada data train
...
*Referensi Opsi Bahasa Inggris:*
  ...
  - [x] We have 33.61% error of our train data
...
*Referensi Opsi Bahasa Inggris:*
  ...
  - [x] 95.11% means that the model performance is good in classifying both positive and negative class
...
*Referensi Opsi Bahasa Inggris:*
  ...
  - [x] Decision Tree because a decision tree model is easily translatable to a set of rules
...
*Referensi Opsi Bahasa Inggris:*
  ...
  - [x] Underfitting is a condition where a model performs poor in the training data but performs well on the test data.
